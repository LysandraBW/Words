...
import { createForm, Form, getFormData, updateForm } from "@/utilities/form";
import z from "zod";
...
export default function CreateBook(props: CreateBookProps) {
    ...
    useEffect(() => {
        ...
        let updatedForm = updateForm(form, "book_name", book?.volumeInfo?.title || "");
        updatedForm = updateForm(updatedForm, "book_cover_image", book?.volumeInfo?.imageLinks?.thumbnail || "");
        updatedForm = updateForm(updatedForm, "book_year", book?.volumeInfo?.publishedDate?.slice(0, 4) || "");
        updatedForm = updateForm(updatedForm, "book_author", book?.volumeInfo?.authors || "");
        setForm(updatedForm);
    }, [selectedBookID]);
        ...
        >
            <div className="px-8 py-4 flex flex-col gap-y-4">
                ...
                <InputText
                    ...
                    onChange={(value) => setForm(updateForm(form, "book_name", value))}
                    required={true}
                ...
                <InputText
                    ...
                    onChange={(value) => setForm(updateForm(form, "book_year", value))}
                    error={form.book_year.error}
                />
                <InputTags
                    label="Author"
                    value={form.book_author.value}
                    onDelete={(value) => setForm(updateForm(form, "book_author", onDeleteAuthor(value)))}
                    onInsert={(value) => setForm(updateForm(form, "book_author", onInsertAuthor(value)))}
                    error={form.book_author.error}
                />
                <InputImageURL
                    label="Cover Image"
                    value={form.book_cover_image.value}
                    onChange={(value) => setForm(updateForm(form, "book_cover_image", value))}
                    error={form.book_cover_image.error}
            />
                <InputImageURL
                    label="Background Image"
                    value={form.book_background_image.value}
                    onChange={(value) => setForm(updateForm(form, "book_background_image", value))}
